...
import rospy
from global_path_plan.srv import PathPlanningPlugin, PathPlanningPluginResponse
...
def make_plan(req):
  ...
  if not path:
    rospy.logwarn("No path returned by A_star's shortes path algorithm")
    path = []
  else:
    execution_time = rospy.Time.now() - start_time
    print("\n")
    rospy.loginfo('++++++++ A_star execution metrics ++++++++')
...
if __name__ == '__main__':
  rospy.init_node('A_star_path_planning_service_server', log_level=rospy.INFO, anonymous=False)
